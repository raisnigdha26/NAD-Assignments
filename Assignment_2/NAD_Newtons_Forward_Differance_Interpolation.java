...
public class NAD_Newtons_Forward_Differance_Interpolation {
     ...
     static int factorial(int n)
    {
     ...
     static float[][] difference(float[] y_arr, int n)
     {
       ...
       static void NFDIF(float x_arr[], float y_arr[], float x, int n)
    {
        ...
        float result = y_arr[1];
        for(int i=1;i<=n-1;i++) {

            result += (numerator(u, i) / factorial(i)) * d[1][i];
